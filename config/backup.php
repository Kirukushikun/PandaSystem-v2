...
return [
    ...
    'backup' => [
         ...
         * the backups.
         */
        // Empty string avoids an extra APP_NAME folder level on every destination disk.
        'name' => '',

        'source' => [
            'files' => [
                /*
                 * Database-only: PANDA's real files (attachments, e-signs) already live on
                 * their own private disk behind policy-gated download routes — zipping the
                 * whole app tree here would be redundant and slow. Leave this empty.
                 */
                'include' => [],
        ...
        'destination' => [
             ...
             */
            // 'google' only joins the run once Drive OAuth credentials are actually set —
            // see App\Providers\GoogleDriveServiceProvider. Local-only until then, and a
            // Drive hiccup (below) never blocks the local copy from being written.
    ...
    'cleanup' => [
        ...
        'default_strategy' => [
            /*
             * One backup a day (18:00) — keep the newest 14 outright, same retention window
             * the old panda:backup command used. No daily/weekly/monthly/yearly thinning on
             * top since there's only ever one backup per day to begin with.
             */
